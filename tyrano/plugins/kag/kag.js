tyrano.plugin.kag = {
    ...
    define: { TYRANO_ENGINE_VERSION: 400, BASE_DIV_NAME: "tyrano_base", FLAG_APRI: !1, www: "" },
    variable: { sf: {}, tf: {} },
    tmp: {
        checking_macro: !1,
        ready_audio: !1,
    ...
    init: function () {
        ...
        $("script").each(function () {
            $(this).attr("src") &&
                ((-1 == $(this).attr("src").indexOf("cordova") && -1 == $(this).attr("src").indexOf("phonegap")) ||
    ...
    init_game: function () {
        ...
        that.variable.sf._system_config_auto_click &&
            (that.config.autoClickStop = that.variable.sf._system_config_auto_click_stop);
        ...
        if (0 != this.stat.log_write) {
            ...
            if (!(max_back_log < 1)) {
                ...
                this.stat.current_save_str =
                    this.variable.tf.system.backlog[this.variable.tf.system.backlog.length - 1];
    ...
    preload: function (src, callbk) {
        ...
        if ("mp3" == ext || "ogg" == ext || "m4a" == ext) {
            let obj = new Howl({ src: src, preload: !0 });
            ...
            obj.on("loaderror", function () {
                that.kag.error(
                    "オーディオファイル「" +
                        src +
                        "」が見つかりません。場所はフルパスで指定されていますか？ (例)data/bgm/music.ogg"
                );
                callbk && callbk();
            });
        } else
    ...
    setStyles: function (j_obj, array_style) {
        for (key in array_style) array_style[key] && ("" == array_style[key] || j_obj.css(key, array_style[key]));
        return j_obj;
    },
