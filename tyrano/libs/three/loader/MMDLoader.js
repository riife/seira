...
THREE.MMDLoader = (function () {
    ...
    _getParser: function () {
      if (this.parser === null) {
        if (typeof MMDParser === 'undefined') {
          throw new Error(
            'THREE.MMDLoader: Import MMDParser https://github.com/takahirox/mmd-parser'
          )
        }
  ...
  GeometryBuilder.prototype = {
    ...
    build: function (data) {
      ...
      for (var i = 0; i < data.metadata.morphCount; i++) {
        ...
        var attribute = new THREE.Float32BufferAttribute(
    ...
    build: function (data, geometry /*, onProgress, onError */) {
      ...
      for (var i = 0; i < data.metadata.materialCount; i++) {
        ...
        if (data.metadata.format === 'pmx' && (material.flag & 0x1) === 1) {
          ...
          params.side =
            params.opacity === 1.0 ? THREE.FrontSide : THREE.DoubleSide
        }
        ...
        if (data.metadata.format === 'pmd') {
          ...
          if (
            material.envTextureIndex !== -1 &&
            (material.envFlag === 1 || material.envFlag == 2)
          ) {
          ...
          if (material.toonIndex === -1 || material.toonFlag !== 0) {
            toonFileName =
              'toon' + ('0' + (material.toonIndex + 1)).slice(-2) + '.bmp'
            isDefaultToon = true
          } else {
  ...
  AnimationBuilder.prototype = {
    ...
    _createTrack: function (
      ...
      if (times.length > 2) {
        ...
        for (
          var aheadIndex = 2, endIndex = times.length;
          aheadIndex < endIndex;
          aheadIndex++
        ) {
  ...
  CubicBezierInterpolation.prototype = Object.assign(
    Object.create(THREE.Interpolant.prototype),
    {
      ...
      _calculate: function (x1, x2, y1, y2, x) {
         ...
         *   http://d.hatena.ne.jp/edvakf/2011101seira/1318716097
         *
         *    x = ( 1 - t ) ^ 3 * x0
         *      + 3 * ( 1 - t ) ^ 2 * t * x1
         *      + 3 * ( 1 - t ) * t^2 * x2
